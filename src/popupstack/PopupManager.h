...
typedef int PopupAnimMode;
enum PopupAnimMode_
{
    PopupAnimMode_None = 0,           // no animation (default)
    PopupAnimMode_Fade = 1 << 0,
    PopupAnimMode_Scale = 1 << 1
};
struct PopupAnimConfig
{
    PopupAnimMode               mode{ PopupAnimMode_Fade | PopupAnimMode_Scale };   // animation mode
...
// Active item on the draw stack
struct ActiveItem
{
    PopupHandle handle{};                           // key into store map
    bool        opened_once{ false };               // ImGui::OpenPopup() armed?
    bool        defer_open{ false };                // delay open until next DrawAll() call [used if parent was closed before child could open]
